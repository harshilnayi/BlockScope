...
""")
    ...
    assert any(keyword in output.lower() for keyword in ["scan", "analyz", "contract"]), \
...
def test_fastapi_scan_endpoint_response_format(test_client, test_contract_code):
    ...
    assert data["vulnerabilities_count"] == len(data["findings"]), \
...
pytest.mark.cli = pytest.mark.skipif(
    not Path("backend/cli/main.py").exists(),
    reason="CLI not available"
)

pytest.mark.fastapi = pytest.mark.skipif(
    not Path("backend/app/main.py").exists(),
