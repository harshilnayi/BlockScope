...
def scan(contract_file, output, contract_name):
    ...
    try:
        ...
        request = ScanRequest(
            source_code=source_code, contract_name=contract_name, file_path=str(file_path)
        )
        ...
        if output == "json":
            ...
            click.secho(
                ...
                fg=(
                    ...
                    else "yellow" if result.overall_score >= 50 else "red"
                ),
                bold=True,
            )
            ...
            if result.severity_breakdown:
                ...
                for severity, count in result.severity_breakdown.items():
                    color = (
                        ...
                        else "yellow" if severity == "high" else "blue"
                    )
